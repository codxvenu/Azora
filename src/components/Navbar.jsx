...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`sticky top-0 z-50 backdrop-blur-xl border-b transition-colors duration-500 dark:bg-zinc-950/80 dark:border-zinc-900 dark:text-white bg-white/80 border-zinc-100`}
      >
          ...
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center gap-8">
              ...
              <div className="hidden md:flex items-center gap-6">
                <NavButton active={window.location.href.includes("/")} to={"/"}>
                  Browse
                </NavButton>
                <NavButton
                ...
                {user?.role === "admin" && (
                  <NavButton active={window.location.href.includes("/seller")}>
                    Sell
                  </NavButton>
                )}
                {user?.role === "admin" && (
            ...
            <div className="flex items-center gap-4">
              ...
              {!!user ? (
                <div className="flex items-center gap-4">
                  <Link to="wallet">
                    ...
                    >
                      <Wallet
                        className={`w-4 h-4 transition-colors dark:text-zinc-400 dark:group-hover:text-white text-zinc-500 group-hover:text-black`}
                      />
                  ...
                  <div className="relative group">
                    <button
                      className={`w-10 h-10 rounded-xl flex items-center justify-center border border-zinc-200 overflow-hidden transition-colors dark:bg-zinc-900 dark:border-zinc-800 bg-zinc-50 border-zinc-200
                        `}
                    >
                      {user?.img ? (
                        <img
                    ...
                    <div
                      className={`absolute right-0 mt-2 w-48 rounded-2xl shadow-xl border py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all dark:bg-zinc-950 dark:border-zinc-900 dark:shadow-black/80 dark:text-white bg-white border-zinc-100
                        `}
                    >
                      <Link to={"/profile"}>
                        <button
              ...
              <button
                className={`md:hidden p-1 rounded-lg dark:text-white dark:hover:bg-zinc-900 text-zinc-950 hover:bg-zinc-100`}
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="md:hidden fixed inset-0 z-40 dark:bg-black bg-white pt-20 px-6"
          >
